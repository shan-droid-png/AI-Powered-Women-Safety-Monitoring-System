...
def gen_frames():
    global prev_gray, video_stream, alarm_triggered, violence_duration, total_frames, violent_frames, non_violent_frames
    ...
    while video_stream.isOpened():
        ...
        for (x, y, w, h) in faces:
            ...
            confidence = gender_preds[0].max()  # Get the confidence score
            detected_genders.append(gender)
            label = f"{gender} ({confidence * 100:.2f}%)"  # Display confidence percentage
        ...
        cv2.putText(frame, count_label, (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)  # Blue color
...
def alarm_system():
    ...
    while True:
        if alarm_triggered:
            ...
            try:
                pygame.mixer.music.load(r"D:\SIH-2024-Project---Women-Safety-USing-CCTV--main (2)\flask\alarm.mp3")
...
@app.route('/analytics')
def analytics():
    global total_frames, violent_frames, non_violent_frames
    if total_frames > 0:
        violent_percentage = (violent_frames / total_frames) * 100
        non_violent_percentage = (non_violent_frames / total_frames) * 100
    else:
        violent_percentage = 0
        non_violent_percentage = 0
    return jsonify({'violent_percentage': violent_percentage, 'non_violent_percentage': non_violent_percentage})
